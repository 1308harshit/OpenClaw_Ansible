...
const TOOL_DECLARATIONS = [
  ...
  },
  {
    name: "run_playbooks",
    description:
      "Run multiple playbooks in sequence with a single call. Use this when the user asks for multiple operations in one prompt (e.g. 'show interfaces AND check VLANs AND harden the fabric'). Returns per-playbook results. Prefer this over calling run_playbook multiple times for combo prompts.",
    parameters: {
      type: "object",
      properties: {
        playbooks: {
...
// SSE streaming endpoint — emits progress events as tool calls happen
app.get("/query-stream", async (req, res) => {
  ...
  try {
    ...
    send({ type: "result", text: out.text || "" });
  } catch (e) {
